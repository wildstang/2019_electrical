...
// Hard codes the calibrated max/min for immediate use at an FRC competition
void QTRSensors::calibratePreset(unsigned int *calibratedMaxList, unsigned int *calibratedMinList, unsigned char readMode)
{
    if(readMode == QTR_EMITTERS_ON_AND_OFF || readMode == QTR_EMITTERS_ON)
    {
      setCalibratedVals(calibratedMaxList, calibratedMinList, &calibratedMaximumOn,
                        &calibratedMinimumOn, QTR_EMITTERS_ON);
    }
    if(readMode == QTR_EMITTERS_ON_AND_OFF || readMode == QTR_EMITTERS_OFF)
    {
      setCalibratedVals(calibratedMaxList, calibratedMinList, &calibratedMaximumOff,
                        &calibratedMinimumOff, QTR_EMITTERS_OFF);
    }
}

void QTRSensors::setCalibratedVals(unsigned int *maxValsList,
                                   unsigned int *minValsList,
                                   unsigned int **calibratedMinimum,
                                   unsigned int **calibratedMaximum,
                                   unsigned char readMode)
{
    int i;
    unsigned int max_sensor_values[64];
    unsigned int min_sensor_values[64];
    ...
    for(i=0;i<_numSensors;i++)
    {
            (*calibratedMaximum)[i] = minValsList[i];
            (*calibratedMinimum)[i] = maxValsList[i];
    }
}
