...
class QTRSensors
{
  public:
    ...
    // Meant for use at FRC events after an initial calibration has been done.
    // Max and min from a previous calibration should be stored manually and entered
    // as parameters to this funciton.
    void calibratePreset(unsigned int *calibratedMaxList, unsigned int *calibratedMinList, unsigned char readMode = QTR_EMITTERS_ON);
  ...
  private:
    ...
    void setCalibratedVals(unsigned int *maxValsList,
